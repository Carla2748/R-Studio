...
```{r, echo=TRUE}
totalstep2 <- newds %>% select(steps,date) %>%
                    group_by(date) %>%
                    summarise(meansteps = mean(steps, na.rm = TRUE),
                              mediansteps = median(steps, na.rm = TRUE))

ggplot(totalstep2,aes(x=steps)) + 
      geom_histogram(fill="navy", bins=30)+
      labs(title = "Daily Steps", x="Steps", y="Frequency")
```
...
```{r, echo=TRUE}
ds3 <- aggregate(steps ~ interval + day, data=ds, mean)
ggplot(ds3, aes(interval, steps)) + geom_line() +
